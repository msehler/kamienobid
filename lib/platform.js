...
function getTestAccountRedirectPath(user) {
  if (!user) {
    return "/account";
  }
  if (user.role === "client") {
...
function listTestAccountsSummary() {
  const allCases = hydrateCases();
  const allBids = hydrateBids();

  const clients = listHydratedUsersByRole("client")
    .filter((user) => isTestAccountEmail(user.email))
    .sort((a, b) => a.email.localeCompare(b.email))
    .map((user) => {
      const matters = allCases.filter((matter) => matter.clientUserId === user.id);
      return {
        id: user.id,
        role: user.role,
        name: user.name,
        ...
        status: user.status,
        caseCount: matters.length,
        liveCaseCount: matters.filter((matter) => matter.status === "open" && String(matter.paymentStatus).startsWith("paid")).length,
        draftCaseCount: matters.filter((matter) => !String(matter.paymentStatus).startsWith("paid")).length,
        engagedCaseCount: matters.filter((matter) => matter.status === "engaged" || matter.acceptedBidId).length,
        latestMatter: matters[0]
          ? {
              id: matters[0].id,
              caseName: matters[0].caseName || getPracticeArea(matters[0].practiceAreaId).label,
              practiceAreaLabel: getPracticeArea(matters[0].practiceAreaId).label,
            }
          : null,
        redirectPath: getTestAccountRedirectPath(user),
      };
    });
  ...
  const lawyers = listHydratedUsersByRole("lawyer")
    ...
    .sort((a, b) => a.email.localeCompare(b.email))
    .map((user) => {
      const bids = allBids.filter((bid) => bid.lawyerId === user.id);
      return {
        id: user.id,
        role: user.role,
        name: user.name,
        ...
        status: user.status,
        firm: user.firm,
        lawyerRole: user.lawyerRole,
        jurisdictions: user.jurisdictions,
        bidCount: bids.length,
        latestBidCaseId: bids[0]?.caseId || null,
        redirectPath: getTestAccountRedirectPath(user),
      };
    });

  return {
    clientPassword: TEST_ACCOUNT_PASSWORDS.client,
    lawyerPassword: TEST_ACCOUNT_PASSWORDS.lawyer,
    clients,
    lawyers,
  };
}
...
function createCase(viewer, payload) {
  ...
  const missing = [];
  ["caseName", "countryCode", "practiceAreaId", "summary", "region"].forEach((field) => {
    if (!payload[field]) {
      missing.push(field);
    }
  });
  ...
  let documents = [];
  try {
    documents = normalizeCaseDocuments(payload.documents, { strict: true });
  } catch (error) {
  ...
  run(
    ...
    [
      ...
      normalizeScopeOfWork(payload.scopeOfWork, payload.practiceAreaId),
      payload.summary,
      payload.budget || "",
...
function updateCase(viewer, payload) {
  ...
  if (matter.clientUserId !== viewer.id) {
    return { status: 403, body: { error: "You can only update your own matters." } };
  }
  if (matter.status === "engaged" || matter.acceptedBidId) {
    return { status: 409, body: { error: "Accepted matters cannot be edited." } };
  }
  ...
  run(
    `UPDATE cases
     SET case_name = ?, country_code = ?, region = ?, practice_area_id = ?, quote_mode = ?,
         summary = ?, budget = ?, documents_json = ?, custom_answers_json = ?, fee_amount = ?
     WHERE id = ?`,
    [
      payload.caseName,
      payload.countryCode,
...
function deleteCase(viewer, payload) {
  const matter = hydrateCaseRow(getCaseRowById(payload.caseId));
  if (!matter) {
    return { status: 404, body: { error: "Matter not found." } };
  }
  if (matter.clientUserId !== viewer.id) {
    return { status: 403, body: { error: "You can only delete your own matters." } };
  }
  if (matter.status === "engaged" || matter.acceptedBidId) {
    return { status: 409, body: { error: "Accepted matters cannot be deleted." } };
  }

  run("DELETE FROM cases WHERE id = ?", [matter.id]);
  return { status: 200, body: { message: "Matter deleted" } };
}
...
function updateLawyerProfile(viewer, payload) {
  ...
  run("UPDATE users SET name = ?, firm = ?, status = ? WHERE id = ?", [
    normalizeName(payload.name || viewer.name),
...
function completeLawyerRegistration(viewer, payload) {
  const lawyerRole = String(payload.lawyerRole || viewer.lawyerRole || "").trim();
  const firmName = String(payload.firmName || "").trim().replace(/\s+/g, " ").slice(0, 160);
  const firmAddress = String(payload.firmAddress || "").trim();
  const firmPhone = String(payload.firmPhone || "").trim().replace(/\s+/g, " ").slice(0, 40);
  const firmContactName = normalizeName(payload.firmContactName || "");
  const lawyerRegion = String(payload.lawyerRegion || "").trim();
  const lawyerCountryCode = String(payload.firmCountryCode || payload.lawyerCountryCode || "").trim();
  const identityDocuments = Array.isArray(payload.identityDocuments)
    ? payload.identityDocuments.filter((entry) => entry?.name && entry?.dataUrl)
    : [];
  const practisingCertificate = payload.practisingCertificate?.name && payload.practisingCertificate?.dataUrl
    ? payload.practisingCertificate
    : null;
  const regulatorConsent = Boolean(payload.regulatorConsent);

  if (!["Employee", "Principal Lawyer", "Partner"].includes(lawyerRole)) {
    return { status: 422, body: { error: "Select your role at the firm." } };
  }
  if (!lawyerRegion || !lawyerCountryCode) {
    return { status: 422, body: { error: "Region of practice is required." } };
  }
  if (!firmName || !firmAddress || !firmPhone || !firmContactName) {
  ...
  run(
    `UPDATE users
     SET lawyer_role = ?, firm = ?, firm_address = ?, firm_phone = ?, firm_contact_name = ?,
         identity_documents_json = ?, practising_certificate_json = ?, regulator_consent = ?, status = ?
     WHERE id = ?`,
    [
      lawyerRole,
      firmName,
      ...
      JSON.stringify(practisingCertificate),
      1,
      "pending",
      viewer.id,
    ],
  ...
  run(
    "INSERT INTO user_jurisdictions (id, user_id, country_code, region, certificate_ref, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    [createId("jur"), viewer.id, lawyerCountryCode, lawyerRegion, "", new Date().toISOString()],
  );

  return {
    status: 200,
    body: {
      message: "Lawyer registration completed and sent for review.",
      lawyer: getHydratedUserById(viewer.id),
    },
  };
}
...
function createBid(viewer, payload) {
  ...
  if (totalWords < 120) {
    return { status: 422, body: { error: `Structured estimate must contain at least 120 words. Current count: ${totalWords}.` } };
  }
    ...
    return {
      ...
      body: {
        ...
        engagementLetter: {
          ...
          ).label}. Pricing approach: ${bid.feeType}. Likely overall range: ${bid.totalFee}. This draft is jurisdiction-aware and should be reviewed before signature.`,
        },
      },
    };
  }
...
function getAdminSummary() {
  return {
    ...
    analytics: summarizeAnalytics(),
    testAccounts: listTestAccountsSummary(),
  };
}

function updateAdmin(viewer, payload) {
  ...
  if (payload.action === "seed-test-accounts") {
    seedTestAccounts({ force: true });
    return {
      status: 200,
      body: {
  ...
  if (payload.action === "impersonate-user") {
    const user = getHydratedUserById(payload.userId);
    if (!user || !isTestAccountEmail(user.email) || user.role === "admin") {
      return { status: 404, body: { error: "Test account not found." } };
    }
    return {
      status: 200,
      body: {
  ...
  if (payload.action === "update-admin-credentials") {
    const adminUser = getUserRowById(viewer?.id);
    if (!adminUser || adminUser.role !== "admin") {
      return { status: 404, body: { error: "Admin account not found." } };
    }

    const normalizedEmail = normalizeEmail(payload.email || adminUser.email);
    const currentPassword = String(payload.currentPassword || "");
    const newPassword = String(payload.newPassword || "");
    const confirmPassword = String(payload.confirmPassword || "");
    const emailChanged = normalizedEmail !== adminUser.email;
    const passwordChangeRequested = Boolean(currentPassword || newPassword || confirmPassword);

    if (!normalizedEmail) {
      return { status: 422, body: { error: "Admin email is required." } };
    }

    const existingByEmail = getUserByEmail(normalizedEmail);
    if (existingByEmail && existingByEmail.id !== adminUser.id) {
      return { status: 409, body: { error: "Another account is already using that email address." } };
    }

    let nextPasswordHash = adminUser.password_hash;

    if (emailChanged || passwordChangeRequested) {
      if (!currentPassword) {
        return { status: 422, body: { error: "Enter your current password to update the admin sign-in details." } };
      }
    ...
    if (passwordChangeRequested) {
      if (!newPassword || !confirmPassword) {
        return { status: 422, body: { error: "Enter and confirm the new password." } };
      }
      if (newPassword.length < 10) {
        return { status: 422, body: { error: "New admin password must be at least 10 characters." } };
      }
      if (newPassword !== confirmPassword) {
        return { status: 422, body: { error: "New password and confirmation do not match." } };
      }
      if (verifyPassword(newPassword, adminUser.password_hash)) {
        return { status: 422, body: { error: "Choose a password that is different from the current one." } };
      }
      nextPasswordHash = hashPassword(newPassword);
    }

    run("UPDATE users SET email = ?, password_hash = ? WHERE id = ?", [normalizedEmail, nextPasswordHash, adminUser.id]);
    return {
      status: 200,
      body: {
        message: passwordChangeRequested ? "Admin email and password updated." : "Admin email updated.",
        user: getHydratedUserById(adminUser.id),
      },
    };
  }
